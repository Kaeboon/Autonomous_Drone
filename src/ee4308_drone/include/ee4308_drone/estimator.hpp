...
namespace ee4308::drone
{
    ...
    private:
        ...
        Eigen::Vector3d Xzb_ = {0, 0, 0};
        // Eigen::Matrix2d Px_ = Eigen::Matrix2d::Constant(1e3),
        //                 Py_ = Eigen::Matrix2d::Constant(1e3),
        //                 Pa_ = Eigen::Matrix2d::Constant(1e3);
        Eigen::Matrix3d Pzb_ = Eigen::Matrix3d::Constant(1e3);
    ...
    private:
        ...
        void correctFromMagnetic(const geometry_msgs::msg::Vector3Stamped msg)
        {
            ...
            //std::cout<<"vector x"<< msg.vector.x <<std::endl;
            //std::cout<<"vector y"<< msg.vector.y <<std::endl;
            //std::cout<<"Ymagnet_"<<Ymagnet_<<std::endl;
            //std::cout<<"Xa_"<<Xa_<<std::endl;
        ...
        void correctFromBaro(const geometry_msgs::msg::PointStamped msg)
        {
            ...
            // Xzb_ = Xzb_ + KBaro * (Ybaro_ - Xzb_[0] - Xzb_[2]); 
            // Xz_ = Xz_ + KBaro * (Ybaro_ - Xzb_[0] - Xzb_[2]);
            // Pz_ = Pz_ - ((KBaro*H)*Pz_);
        ...
        void predict(const sensor_msgs::msg::Imu msg)
        {
            ...
            // degree of freedom z for barometer;
            // Eigen::Matrix3d Fzbmatrix = Eigen::Matrix2d::Constant(1e3);
            // Fzbmatrix(0,0) = 1;
            // Fzbmatrix(0,1) = dt;
            // Fzbmatrix(0,2) = 0;
            // Fzbmatrix(1,0) = 0;
            ...
            // Eigen::Vector3d Wzbvector = {0.5 * dt * dt, dt, 0};
            
            // Xzb_ = (Fzbmatrix * Xzb_) + (Wzbvector * (uz-G));

            // Pzb_ = (Fzbmatrix * Pzb_ * Fzbmatrix.transpose()) + (Wzbvector * varz * Wzbvector.transpose());
